...
function getErrorTypeFromMessage(message: string): AuthErrorType {
  ...
  if (lowerMessage.includes("server") || lowerMessage.includes("internal") || lowerMessage.includes("unavailable")) {
    return AuthErrorType.SERVER;
  }
  return AuthErrorType.UNKNOWN;
}
...
function getFriendlyMessage(message: string): string {
  ...
  if (lowerMessage.includes("credentials") || (lowerMessage.includes("email") && lowerMessage.includes("password"))) {
